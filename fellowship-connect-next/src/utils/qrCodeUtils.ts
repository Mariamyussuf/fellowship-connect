import type { QRCodeSession, AttendanceRecord } from '../types';
...
export const validateQRCodeSession = (
  ...
  try {
    ...
    if (!sessionData.expiresAt) {
      return { isValid: false, error: 'QR code data is missing expiration time' };
    }
    ...
    if (!sessionData.timestamp) {
      return { isValid: false, error: 'QR code data is missing timestamp' };
    }
...
// Define interface for offline token data
...
export const validateOfflineToken = (token: string): { isValid: boolean; data?: OfflineTokenData } => {
  try {
    const data: OfflineTokenData = JSON.parse(atob(token));
