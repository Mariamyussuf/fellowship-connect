...
export class OfflineService {
  ...
  async storeData<T>(storeName: keyof FellowshipDB, data: T[]): Promise<void> {
  ...
  async getData<T>(storeName: keyof FellowshipDB, key?: string): Promise<T[]> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async clearOfflineData(): Promise<void> {
    ...
    const storeNames: (keyof FellowshipDB)[] = [
